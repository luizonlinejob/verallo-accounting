<template>
  <div class="space-y-6 font-sans">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div class="p-6 bg-slate-50/60 border-b border-slate-100">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div class="md:col-span-3">
            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-wider">Course / Department</label>
          ...
          <div :class="period === 'custom' ? 'md:col-span-3' : 'md:col-span-6'" class="flex items-end">
            <button
    ...
    <div v-if="report">
      ...
      <div v-if="report.accounting_summary" class="mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl p-5 text-white shadow-lg">
            ...
            <p class="text-xs text-emerald-200 mt-1">{{ report.accounting_summary.collection_rate }}% collection rate</p>
          </div>

          <div class="bg-gradient-to-br from-rose-600 to-rose-800 rounded-2xl p-5 text-white shadow-lg">
            <p class="text-[10px] font-bold uppercase tracking-widest text-rose-200">Total Receivable</p>
          ...
          <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-5 text-white shadow-lg">
            <p class="text-[10px] font-bold uppercase tracking-widest text-blue-200">Total Assessment</p>
          ...
          <div class="bg-gradient-to-br from-amber-600 to-amber-800 rounded-2xl p-5 text-white shadow-lg">
            <p class="text-[10px] font-bold uppercase tracking-widest text-amber-200">Pending</p>
      ...
      <div v-if="report.per_course && report.per_course.length > 0" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white flex justify-between items-center">
          <h3 class="font-bold text-slate-800 flex items-center gap-2">
            <span>📊</span> I. Monthly Collection Summary per Course
          </h3>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-slate-50/80">
              <tr>
                <th class="px-4 py-3 text-left text-[11px] font-bold text-gray-500 uppercase">Course / Department</th>
                <th class="px-4 py-3 text-center text-[11px] font-bold text-gray-500 uppercase">Total Students</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Assessment</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Monthly Collection</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Receivable</th>
                <th class="px-4 py-3 text-center text-[11px] font-bold text-gray-500 uppercase">Rate</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-sm bg-white">
              ...
              <tr class="bg-slate-100 font-bold">
                <td class="px-4 py-3 text-xs uppercase">Grand Total</td>
                ...
                <td class="px-4 py-3 text-center font-mono">{{ overallCourseRate }}%</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <!-- II. (REMOVED) Monthly Revenue Breakdown by Fee Category -->
      <!-- III. (REMOVED) Accounts Receivable Aging Report -->

      <!-- IV. ENROLLMENT AND FEE REALIZATION REPORT -->
      <div v-if="report.enrollment_realization && report.enrollment_realization.length > 0" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
          <h3 class="font-bold text-slate-800 flex items-center gap-2">
            <span>🎯</span> II. Enrollment and Fee Realization Report
          </h3>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-slate-50/80">
              <tr>
                ...
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Actual Collection</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Variance</th>
                <th class="px-4 py-3 text-center text-[11px] font-bold text-gray-500 uppercase">Realization</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-sm bg-white">
        ...
        <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-white flex justify-between items-center gap-3">
          ...
          <div class="flex items-center gap-2">
            <span class="text-xs text-slate-500">
              Total Balance: <strong class="text-rose-700 font-mono">₱{{ formatMoney(report.student_list.total_balance) }}</strong>
            </span>
            <button
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-slate-50/80">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-[11px] font-bold text-gray-500 uppercase">Name of Students</th>
                <th class="px-4 py-3 text-left text-[11px] font-bold text-gray-500 uppercase">Course</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Total Assessment</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Total Receivables</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Payment</th>
                <th class="px-4 py-3 text-right text-[11px] font-bold text-gray-500 uppercase">Balance</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-sm bg-white">
              <tr
              ...
              >
                ...
                <td class="px-4 py-3 text-right font-mono text-rose-700">₱{{ formatMoney(student.total_receivables) }}</td>
                <td class="px-4 py-3 text-right font-mono font-bold text-emerald-700">₱{{ formatMoney(student.total_paid) }}</td>
                <td class="px-4 py-3 text-right font-mono font-bold" :class="student.balance > 0 ? 'text-rose-700' : 'text-emerald-700'">
      ...
      <!-- VI. COLLECTION BY PAYMENT METHOD -->
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
          <h3 class="font-bold text-slate-800 flex items-center gap-2">
            <span>💵</span> IV. Collection by Payment Method
          </h3>
        </div>
        <div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <div v-for="(m, i) in report.method_breakdown" :key="i" class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="font-semibold text-slate-800 text-sm capitalize">{{ (m.method || 'cash').replace('_', ' ') }}</p>
                <p class="text-xs text-slate-500">{{ m.count }} txn</p>
              </div>
            </div>
            <span class="font-mono font-bold text-emerald-700 text-sm">₱{{ formatMoney(m.total) }}</span>
          </div>
          <div v-if="report.method_breakdown.length === 0" class="col-span-full p-6 text-center text-slate-400 italic text-sm">
            No payment data.
          </div>
        </div>
      </div>

      <!-- VII. TRANSACTION DETAILS -->
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white flex justify-between items-center">
          <h3 class="font-bold text-slate-800 flex items-center gap-2">
            <span>📋</span> V. Transaction Details
          </h3>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 text-sm bg-white">
              <tr v-for="t in report.transactions" :key="t.id" class="hover:bg-indigo-50/30 transition">
                ...
                <td class="px-4 py-3 text-center">
                  <span :class="statusClass(t.status)" class="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">{{ t.status }}</span>
                </td>
              </tr>
...
const totalCourseStudents = computed(() =>
  report.value?.per_course?.reduce((s, c) => s + c.student_count, 0) || 0
);
const totalCourseAssessment = computed(() =>
  report.value?.per_course?.reduce((s, c) => s + c.total_assessment, 0) || 0
);
const totalCourseCollection = computed(() =>
  report.value?.per_course?.reduce((s, c) => s + c.collection_in_range, 0) || 0
);
const totalCourseReceivable = computed(() =>
  report.value?.per_course?.reduce((s, c) => s + c.receivable, 0) || 0
);
...
/**
 * 🖨️ Print ONLY the Student List section (walay Grand Total row)
 */
const printStudentList = () => {
  if (!report.value || !report.value.student_list) return;

  const students = report.value.student_list.students;
  const courseName = report.value.course_filter || 'All Courses';
  ...
  const rows = students.map((s, i) => `
    <tr>
      ...
      <td style="padding:8px;border:1px solid #cbd5e1;text-align:right;font-family:monospace;font-size:11px;color:#dc2626;">₱${formatMoney(s.total_receivables)}</td>
      <td style="padding:8px;border:1px solid #cbd5e1;text-align:right;font-family:monospace;font-size:11px;color:#059669;">₱${formatMoney(s.total_paid)}</td>
      <td style="padding:8px;border:1px solid #cbd5e1;text-align:right;font-family:monospace;font-size:11px;font-weight:bold;color:${s.balance > 0 ? '#dc2626' : '#059669'};">₱${formatMoney(s.balance)}</td>
    </tr>
  `).join('');
  ...
  const html = `
    ...
    <body>
      ...
      <div class="report-title">
        ...
        <p>${report.value.student_list.student_count} student(s) enrolled</p>
      </div>

      <!-- SUMMARY BOX -->
      <div class="summary-box">
        <div class="item">
          <div class="lbl">Total Assessment</div>
          <div class="val" style="color:#1e40af;">₱${formatMoney(report.value.student_list.total_assessment)}</div>
        </div>
        <div class="item">
          <div class="lbl">Total Receivables</div>
          <div class="val" style="color:#dc2626;">₱${formatMoney(report.value.student_list.total_balance)}</div>
        </div>
        <div class="item">
          <div class="lbl">Total Payment</div>
          <div class="val" style="color:#059669;">₱${formatMoney(report.value.student_list.total_paid)}</div>
        </div>
        <div class="item">
      ...
      <!-- STUDENT TABLE (walay Grand Total) -->
      <table>
        <thead>
          <tr>
